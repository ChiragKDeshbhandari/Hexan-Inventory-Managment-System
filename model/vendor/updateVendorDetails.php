<?php
	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		$vendorDetailsVendorPinCode = htmlentities($_POST['vendorDetailsVendorPinCode']);
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					$updateVendorDetailsSql = 'UPDATE vendor SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, address2 = :address2, pincode = :pincode, district = :district, status = :status WHERE vendorID = :vendorID';
					$updateVendorDetailsStatement = $conn->prepare($updateVendorDetailsSql);
					$updateVendorDetailsStatement->execute(['fullName' => $vendorDetailsVendorFullName, 'email' => $vendorDetailsVendorEmail, 'mobile' => $vendorDetailsVendorMobile, 'phone2' => $vendorDetailsVendorPhone2, 'address' => $vendorDetailsVendorAddress, 'address2' => $vendorDetailsVendorAddress2, 'pincode' => $vendorDetailsVendorPinCode, 'district' => $vendorDetailsVendorDistrict, 'vendorID' => $vendorDetailsVendorID, 'status' => $vendorDetailsStatus]);
