<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		$pincode = htmlentities($_POST['vendorDetailsVendorPinCode']);
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			// Validate address, address2 and PinCode
			// Validate address
			if($address == ''){
				// Address 1 is empty
			...
			$sql = 'INSERT INTO vendor(fullName, email, mobile, phone2, address, address2, pincode, district, status) VALUES(:fullName, :email, :mobile, :phone2, :address, :address2, :pincode, :district, :status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'phone2' => $phone2, 'address' => $address, 'address2' => $address2, 'pincode' => $pincode, 'district' => $district, 'status' => $status]);
